...
!C      open (13,file='s2.out',status='unknown')
...
!C>>>**************************************************************** Unstady Loop - START
      do ITERkk= 1, ITERmaxTIME
        ...
        if (my_rank.eq.0) then 
          ...
          do 
!C            read (12,*,end=200) ttx, Source
             dest=0
             call WAITIO_MPI_Irecv (ttx, 1,  
...
!c     &     Source
          ...
          do 
!C            read (13,*,end=100) ttt, tt1, s0, s1, s2, TBOU
             dest= 1
             call WAITIO_MPI_Irecv (ttt, 1,  
     &            WAITIO_MPI_DOUBLE_PRECISION,      
     &            dest, 0, WAITIO_SOLVER_COMM, req1(1,1), ierr)
             call WAITIO_MPI_Irecv (tt1, 1,  
     &            WAITIO_MPI_DOUBLE_PRECISION,      
     &            dest, 0, WAITIO_SOLVER_COMM, req1(1,2), ierr)
             call WAITIO_MPI_Irecv (s0, 1,  
     &            WAITIO_MPI_DOUBLE_PRECISION,      
     &            dest, 0, WAITIO_SOLVER_COMM, req1(1,3), ierr)
             call WAITIO_MPI_Irecv (s1, 1,  
     &            WAITIO_MPI_DOUBLE_PRECISION,      
     &            dest, 0, WAITIO_SOLVER_COMM, req1(1,4), ierr)
             call WAITIO_MPI_Irecv (s2, 1,  
     &            WAITIO_MPI_DOUBLE_PRECISION,      
     &            dest, 0, WAITIO_SOLVER_COMM, req1(1,5), ierr)
             call WAITIO_MPI_Irecv (TBOU, 1,  
     &            WAITIO_MPI_DOUBLE_PRECISION,      
     &            dest, 0, WAITIO_SOLVER_COMM, req1(1,6), ierr)
          ...
          enddo
 100      continue
!c             write (*, '(/a,6(1pe16.6))') '<< get from wrank=1 ', ttt, 
!c     &        tt1, s0, s1, s2, TBOU
        endif
